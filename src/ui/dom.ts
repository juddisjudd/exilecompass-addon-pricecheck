 ...
 *
 * Layout rule for this panel: the listings own the vertical space. The
 * overlay's default window leaves the panel about 490px square, so
 * everything above the listings (input, item, filters) collapses to a line
 * once it has served its purpose, and each listing is one line until opened.
 * The answer to "what is this worth" is the prices, not the item you already
 * have in your hand.
 *
 * Sizes: chrome (section labels, footer, sort buttons) sits at 10–11px; the
 * answer — listing names, prices, mod text — at 12px, the host's own body
 * size.
 */
export const CSS = `
  ...
  /* Scoped under .pc so it outranks the generic .pc button rule above; a
     bare .pc-link lost to it and drew as a boxed button. */
  .pc .pc-link { background:none; border:none; padding:0; color:var(--c-accent);
    text-decoration:underline; cursor:pointer; font-size:10.5px; }
  .pc .pc-link:hover { color:var(--c-primary); }
  ...
  .pc-item-base, .pc-item-class { color:var(--c-accent); font-size:11px; }
  ...
  .pc-chip { font-size:10.5px; color:var(--c-primary); background:rgba(167,154,133,.1);
  ...
  /* ── filters ─────────────────────────────────────────────────────────── */
  /* Takes only the height it needs, shrinking (and scrolling) when the pane is
     short, so Search sits directly under the filters rather than at the bottom
     of whatever space is left. */
  .pc-filters { flex:0 1 auto; display:flex; flex-direction:column; min-height:0; }
  .pc-filters-head { display:flex; align-items:center; gap:6px; flex:0 0 auto; }
  ...
  .pc-filter-list { flex:0 1 auto; min-height:0; overflow-y:auto;
    border:1px solid rgba(167,154,133,.18); border-top:none; }
  ...
  .pc-filter { display:flex; gap:5px; align-items:center; min-height:24px;
    padding:2px 6px; border-bottom:1px solid rgba(167,154,133,.1); }
  .pc-filter:hover { background:rgba(167,154,133,.07); }
  .pc-filter input[type=checkbox] { margin:0; flex:0 0 auto; }
  /* Wraps rather than truncates: a mod cut to "+143(124-151) to Evasion ..."
     is the one thing on the row you have to be able to read. */
  .pc-label { flex:1 1 auto; min-width:0; font-size:11.5px; line-height:1.3;
    overflow-wrap:anywhere; cursor:pointer; }
  .pc-tier { color:var(--c-accent); font-size:9.5px; margin-left:4px; }
  .pc-value { color:var(--c-accent); margin-left:5px; font-variant-numeric:tabular-nums; }
  .pc-value::before { content:':'; margin-right:4px; }
  .pc-label .pc-affix { margin-left:5px; }
  /* The numbers only exist for a filter that is on. */
  .pc-range { display:none; flex:0 0 auto; gap:3px; }
  .pc-filter.on .pc-range { display:flex; }
  .pc-range input[type=number] { width:46px; box-sizing:border-box; padding:1px 3px;
    font-variant-numeric:tabular-nums; }
  .pc-range input[type=number]::placeholder { color:rgba(167,154,133,.6); }
  ...
  .pc-search-wrap { flex:0 0 auto; display:flex; align-items:stretch; gap:5px; }
  .pc-search { flex:1 1 auto; padding:6px; font-size:12px; letter-spacing:.04em; }
  .pc-core { flex:0 0 auto; }
  ...
  .pc-card-body { display:grid; grid-template-columns:minmax(0,1fr) 112px;
    border-top:1px solid rgba(167,154,133,.14); }
  .pc-card-item { padding:5px 8px 7px; min-width:0; }
  ...
  .pc-card-seller { font-size:11.5px; color:var(--c-primary); max-width:100%;
    overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .pc-card-account { font-size:10px; color:var(--c-accent); max-width:100%;
    overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .pc-card-icon { width:34px; height:34px; object-fit:contain; margin-top:4px; }
  .pc-detail-head { display:flex; align-items:baseline; gap:6px; flex-wrap:wrap; }
  .pc-detail-facts { color:var(--c-accent); font-size:10.5px; margin-top:2px; }
  ...
  .pc-mod { display:flex; align-items:baseline; gap:5px; font-size:12px; }
  ...
  .pc-affix { flex:0 0 22px; font-size:10px; font-weight:700; letter-spacing:.02em;
    font-variant-numeric:tabular-nums; }
  ...
  /* The roll range sits right after its mod, not across the card from it. */
  .pc-mod-range { flex:0 0 auto; color:var(--c-accent); font-size:10px;
